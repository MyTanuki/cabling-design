...
// BRANCH_MAX_M / PATCH_MAX_M / PATCH_DETOUR ย้ายไป router.js (BRANCH_MAX_M import กลับมาใช้ในนี้)
...
/* ============================================================
   ...
   (dist / polyLenPx / projToSeg / segIntersect / simplifyPts / samePath /
    nearestOnSegments ย้ายไป router.js แล้ว — import กลับมาใช้ด้านบน)
   ============================================================ */
...
/* ============================================================
   ...
   ตัวหาเส้นทางย้ายไป router.js (createRouter + เรขาคณิต) — ที่นี่เหลือ adapter
   ที่แปลง state ของแอปเป็น input ของ router (wallSegs, buildRouter)
   ============================================================ */
function wallSegs() {
  const segs = [];
...
// สร้าง router จาก state ปัจจุบันของแอป (adapter → createRouter ใน router.js)
function buildRouter(devs) {
  return createRouter(wallSegs(), devs, { pxPerM: state.pxPerM, ortho: $('#chkOrtho').checked });
}
...
function autoPlaceSwitches() {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    const router = buildRouter([...all, ...endpoints.map(e => ({ x: e.x, y: e.y }))]);
...
function showAlternatives(id) {
  ...
  if (!a || !b) return;
  const router = buildRouter([{ x: a.x, y: a.y }, { x: b.x, y: b.y }]);
...
function autoRouteAll() {
  ...
  const router = buildRouter([onu, dsw, ...psws, ...cams, ...waps].filter(Boolean).map(d => ({ x: d.x, y: d.y })));
...
function rerouteAutoForDevice(dev) {
  ...
  const router = buildRouter(state.devices.map(d => ({ x: d.x, y: d.y })));
...
function renderWarnings() {
  ...
  if (state.walls.length && state.pxPerM) {
    const segs = wallSegs();
    const branchPx = BRANCH_MAX_M * state.pxPerM + 0.5; // +epsilon เท่ากับ createRouter
    ...
    const router = state.devices.length ? buildRouter(state.devices.map(d => ({ x: d.x, y: d.y }))) : null;
    for (const r of state.routes) {
